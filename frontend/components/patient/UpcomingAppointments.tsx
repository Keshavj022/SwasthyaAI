...
/** Map a specialty (or appointment type fallback) to a representative icon. */
function specialtyIcon(specialty?: string, type?: string) {
  const key = (specialty || type || '').toLowerCase()
  if (key.includes('cardio') || key.includes('heart')) return Heart
  if (key.includes('neuro') || key.includes('psych')) return Brain
  if (key.includes('ortho') || key.includes('bone')) return Bone
  if (key.includes('pediatric') || key.includes('paed') || key.includes('child')) return Baby
  if (key.includes('ophthal') || key.includes('eye')) return Eye
  if (key.includes('emergency')) return Activity
  return Stethoscope
}
...
function typeBadgeClass(type?: string): string {
  return TYPE_BADGE[(type || '').toLowerCase()] ?? 'bg-gray-100 text-gray-600'
}
      ...
      {upcoming.length === 0 ? (
        ...
        <div className="space-y-3">
          {upcoming.map((appt: Appointment) => {
            const Icon = specialtyIcon(appt.specialty, appt.type)
            return (
              <div
                key={appt.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">
                    ...
                    {appt.specialty && (
                      <span className="text-gray-400 font-normal"> · {appt.specialty}</span>
                    )}
                  </p>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {appt.type && (
                      <span
                        className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium capitalize ${typeBadgeClass(appt.type)}`}
                      >
                        {appt.type}
                      </span>
                    )}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${STATUS_BADGE[appt.status] ?? 'bg-gray-100 text-gray-500'}`}
                  >
                    {appt.status}
                  </span>
                  {confirmId === appt.id ? (

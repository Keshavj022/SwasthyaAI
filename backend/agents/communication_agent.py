...
class CommunicationAgent(BaseAgent):
    ...
    @staticmethod
    def _stub_note() -> str:
        """Standard banner text shown when running on the non-loaded stub."""
        return (
            "AI model not loaded — demo output. This is non-authoritative and "
            "must not be relied upon for medical decisions."
        )
...
"""
    ...
    async def _handle_medical_qa(self, request: AgentRequest) -> AgentResponse:
        ...
        if self.has_inference:
            ...
            stub_mode = response_data.get("stub_mode", True)
            ...
            return AgentResponse(
                ...
                data={
                    ...
                    "stub_mode": stub_mode,
                    "stub_note": self._stub_note() if stub_mode else None,
                    "disclaimer": response_data.get("disclaimer", "")
                },
                confidence=response_data.get("confidence", 0.5),
                reasoning="Generated by MedGemma" if not stub_mode else "MedGemma stub (model not loaded)",
    ...
    async def _handle_simplify(self, request: AgentRequest) -> AgentResponse:
        ...
        if self.has_inference:
            ...
            return AgentResponse(
                ...
                reasoning="Medical text simplified via MedGemma" if not stub_mode else "MedGemma stub (model not loaded)",
                red_flags=response_data.get("red_flags", []),
                requires_escalation=False
            )
